...
function closeAlert() {
  ...
  Dom.setStyle('cancelalert', 'display', 'none');
...
function showAlert(message,autoclose) {
  ...
  Dom.setStyle('cancelalert', 'display', '');
...
function collapseSubFolders(pid) {
  // Remove this folder from the array we use to track expanded folders.
  expandedfolders = arrayRemoveItem(expandedfolders, pid);
...
function togglefolder(id) {
  ...
  if (Dom.hasClass(el, 'icon-folderclosed')) {
    // Add this expanded folder to the array we use to track expanded folders.
...
function expandCollapseFolders(obj,mode) {
  ...
  if (mode == 'expand') {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      Dom.setStyle(elc,'display','none');
      // Remove this folder from the array we use to track expanded folders.
      expandedfolders = arrayRemoveItem(expandedfolders,id);
    }
    obj.href = '?op=expand';
  ...
  if (selectoption == 'delete') {
        ...
        success : function (o) {
          ...
          if (oResults.retcode == 200) {
            if (document.frmtoolbar.reportmode.value == 'notifications') {
              ...
              // Check first to see if listener already active.
              if (!Event.getListeners('clearnotificationhistory')) {
    ...
    var callback = {
      success: function (o) {
        ...
        // Check first to see if listener already active.
        if (!Event.getListeners('btnMoveFolderSubmit')) {
          Event.addListener("btnMoveFolderSubmit", "click", moveSelectedFiles);
        }
        // Check first to see if listener already active.
        if (!Event.getListeners('btnMoveFolderCancel')) {
      ...
      if (oResults.error.length == 0) {
        ...
        if (!oResults.downloadperm) {
          ...
          if (reportmode == 'incoming') {
            download_url += '/incoming';
          }
          if (reportmode == 'approvals') {
            download_url += '/0/moderator';
          }

          Dom.get('menubar_downloadlink').href = download_url;
          // Check first to see if listener already active.
          if (!Event.getListeners('menubar_downloadlink')) {
        ...
        if (!oResults.editperm) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('editfiledetailslink')) {
        ...
        if (!oResults.addperm) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('newversionlink')) {
        ...
        if (!oResults.deleteperm) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('deletefiledetailslink')) {
        ...
        if (!oResults.lockperm) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('lockfiledetailslink')) {
        ...
        if (!oResults.notifyperm) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('notifyfiledetailslink')) {
        ...
        if (oResults.status == 0) {
          ...
          // Check first to see if listener already active.
          if (!Event.getListeners('approvefiledetailslink')) {
...
function showMoveIncomingFile(id) {
  ...
  var callback = {
    success: function (o) {
      ...
      // Check first to see if listener already active.
      if (!Event.getListeners('btnMoveIncomingFileSubmit')) {
        Event.addListener("btnMoveIncomingFileSubmit", "click", moveIncomingFile);
      }
      // Check first to see if listener already active.
      if (!Event.getListeners('btnMoveIncomingFileCancel')) {
...
function showAddFilePanel() {
  ...
  var callback = {
    success: function (o) {
      ...
      // Check first to see if listener already active.
      if (!Event.getListeners('btnNewFileCancel')) {
...
function renderLeftNavigation(oResults) {
  ...
  try {
    // Check first to see if listener already active.
    if (!Event.getListeners('folderoptions_link')) {
...
function renderFileListing(oResults) {
  ...
  if (filedepotfolders == "expanded") {
    ...
    if (linkobj) {
      expandCollapseFolders(linkobj,'expand')
    }
  }
  if (filedepotdetail == 'expanded') {
    showhideFileDetail('show');
  }
  ...
  clearCheckedItems();
  if (document.frmfilelisting.chkfile) {
    // No files in listing - disable the header checkall checkbox.
    if (document.frmfilelisting.chkfile.length == undefined || document.frmfilelisting.chkfile.length == 1) {
...
YAHOO.filedepot.getmorefiledata = function (data) {
  ...
  folderstack = [];
  // PHP Array that we json_encode within the JSON formatted return data to preserve the array.
  lastfiledata = eval('(' + data + ')');
...
YAHOO.filedepot.threadedRequestManager = function (i) {
  ...
  if (folderstack.length > numGetFileThreads) {
    ...
    // Used to track the folders being prcessed.
    folderstack.push(requestdata[0]);
    i++;
    ...
    var callback = {
      success: function (o) {
        ...
        if (oResults.retcode == 200) {
          ...
          // Remove this folder as it's now been processed.
          folderstack = arrayRemoveItem(folderstack,cid);
  ...
  var listingcallback = {
    success: function (o) {
      ...
      if (oResults.retcode == 200) {
        ...
        // Clear any active search tags.
        Dom.get('activesearchtags').innerHTML = '';
        document.fsearch.tags.value = '';
        Dom.setStyle('showactivetags','display','none');
        Dom.get('headerchkall').checked = false;
      ...
      // No permissions to view this category.
      else if (oResults.retcode == 401) {
...
YAHOO.filedepot.alternateRows = {
  ...
  init: function (className) {
    ...
    for (var i = 0; i < recordList.length; i++) {
      setColor = true;
      // This record is a folder so we need to see if it's open or closed.
      if (Dom.hasClass(recordList[i],'subfolder')) {
        // Extract the folder id from the record id assigned.
        var elparts = recordList[i].id.split('subfolder');
        var elc = 'subfolder' + elparts[1] + '_contents';
